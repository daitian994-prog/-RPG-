const LoreVisuals = (() => {
  ...
  function renderCards({mount,category,records,escape,excerpt,meta,onEdit,onDelete,onOfficial,onStory=()=>{},onNewStory=()=>{},officialIds=new Set()}) {
    mount.className='lore-grid';
    mount.innerHTML=records.map(record=>{const tags=meta(record).filter(Boolean).slice(0,3),stories=category==='champions'?(record.stories||[]):[];const image=safeUrl(record.data.image_url),source=safeOfficial(record.data.image_source_url);const protectedRecord=['metadata','region'].includes(category),hasOfficial=officialIds.has(record.id),content=category==='champions'?`<section class="champion-biography"><b>人物传记</b><p>${escape(excerpt(record.data))}</p></section><section class="champion-stories"><header><b>人物故事</b><span>${stories.length} 篇 <button type="button" class="new-champion-story" data-champion-id="${escape(record.id)}">＋ 添加</button></span></header>${stories.length?`<div>${stories.map(story=>`<button type="button" class="champion-story" data-story-id="${escape(story.id)}"><strong>${escape(story.title)}</strong><small>${escape(story.author||'作者未记录')}${story.minutes_to_read?` · ${escape(story.minutes_to_read)} 分钟`:''}</small></button>`).join('')}</div>`:'<p>尚未收录人物故事</p>'}</section>`:`<p>${escape(excerpt(record.data))}</p>`;return `<article class="lore-card media-card ${category==='champions'?'champion-card':''}">${image?`<div class="lore-media" style="background-image:url('${image}')">${source?`<a href="${source}" target="_blank" rel="noreferrer">官方来源 ↗</a>`:''}<span>${escape(record.data.image_credit||'Riot Games 官方素材')}</span></div>`:''}<div class="lore-card-body"><div class="lore-card-top"><span>${escape(category==='champions'?'英雄资料':category)}${hasOfficial?' · 中文官网已同步':''}</span><code>${escape(record.id)}</code></div><h3>${escape(record.title)}</h3>${content}<div class="lore-tags">${tags.map(item=>`<span>${escape(item)}</span>`).join('')}</div><div class="lore-card-actions"><small>更新于 ${escape(record.updated_at)}</small><div>${actionButtons(record,protectedRecord,hasOfficial)}</div></div></div></article>`}).join('');
    wireActions(mount,records,onEdit,onDelete,onOfficial);
    mount.querySelectorAll('.champion-story').forEach(button=>button.onclick=()=>onStory(button.dataset.storyId));
    mount.querySelectorAll('.new-champion-story').forEach(button=>button.onclick=()=>onNewStory(records.find(record=>record.id===button.dataset.championId)));
  }
