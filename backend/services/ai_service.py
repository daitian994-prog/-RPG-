...
class AIService:
    ...
    trait_checks = {
        "peace": ("defense", "防御力＋和平倾向"),
        "power": ("attack", "攻击力＋力量观"),
        "freedom": ("attack_speed", "攻击速度＋自由倾向"),
        "spirit": ("magic_power", "法术强度＋灵性亲和"),
        "destiny": ("skill_haste", "技能急速＋命运态度"),
    }
    ...
    def assess_choice(self, event: dict[str, Any], choice: dict[str, Any], game: dict[str, Any], choice_index: int) -> dict[str, Any]:
        ...
        stat_key, check_label = self.trait_checks[primary_trait]
        player = game["player"]
        stat_value = player["attributes"][stat_key]
        trait_value = player["personality"][primary_trait]
        if event["type"] == "战斗":
            stats = player["attributes"]
            is_boss = event.get("chapter_only", False)
            chance = min(78 if is_boss else 88, (18 if is_boss else 36) + stats["attack"] + stats["defense"] // 2 + stats["magic_power"] // 3 + choice_index * 3)
            forecast = "胜算很高" if chance >= 75 else "胜算较高" if chance >= 60 else "成败难料" if chance >= 42 else "胜算较低"
            return {"primary_trait": primary_trait, "stat": "combat", "check": "综合战斗能力", "difficulty": 70 if is_boss else 60, "risk": "致命" if is_boss else "高", "chance": chance, "forecast": forecast}
        base_difficulty = {"日常": 38, "NPC": 42, "成长": 47, "探索": 50, "命运": 55, "战斗": 62}.get(event["type"], 48)
        if choice.get("result", {}).get("items"):
            base_difficulty += 4
        # Stable per-option variance prevents the third option from always being safest or strongest.
        option_variance = (sum(ord(char) for char in event["id"]) + choice_index * 7) % 9 - 4
        difficulty = base_difficulty + option_variance
        chance = round(48 + (stat_value - 10) * 2 + (trait_value - 40) * 0.35 - (difficulty - 45))
        chance = max(20, min(90, chance))
        risk = "低" if difficulty <= 40 else "中" if difficulty <= 49 else "高" if difficulty <= 57 else "致命"
        forecast = "胜算很高" if chance >= 75 else "胜算较高" if chance >= 60 else "成败难料" if chance >= 42 else "胜算较低"
        return {
    ...
    def evaluate_event_outcome(self, event: dict[str, Any], choice: dict[str, Any], game: dict[str, Any], choice_index: int) -> dict[str, Any]:
        ...
        if roll <= max(5, chance // 5):
            code, label, multiplier = "critical", "大成功", 1.5
        elif roll <= chance:
            code, label, multiplier = "success", "成功", 1.0
        elif roll <= min(96, chance + 25):
            code, label, multiplier = "costly", "代价成功", 0.6
        else:
            code, label, multiplier = "failure", "失败", 0.25

        damage_table = {
            "日常": {"costly": 2, "failure": 5},
            "NPC": {"costly": 0, "failure": 0},
            "成长": {"costly": 5, "failure": 9},
            "探索": {"costly": 7, "failure": 13},
            "命运": {"costly": 8, "failure": 15},
        }
        hp_loss = damage_table.get(event["type"], {}).get(code, 0)
        relationship_factor = 1.3 if code == "critical" else 1.0 if code == "success" else 0.35 if code == "costly" else -0.5
        primary = assessment["primary_trait"]
        tradeoff = self.opposing_traits[primary]
        tradeoff_loss = 0 if code == "failure" else 1 if code in {"success", "costly"} else 2
        setback_lines = {
            "costly": "你达成了最初的目的，却没能全身而退。局势留下的伤口提醒你：正确的方向同样可能索取代价。",
        ...
        return {
            **assessment,
            ...
            "grants_items": code in {"critical", "success", "costly"},
