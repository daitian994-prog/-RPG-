...
当前版本：**v0.4.1**。版本创建、离线备份与安全恢复方法见 [版本说明.md](版本说明.md)。当前功能、技术结构、网页端接口约定和完整更新日志可在 **API节点管理器 → 项目总览与日志** 中查看，也可由 `GET /api/admin/project-status` 读取。

阶段5已将叙事权限拆成程序权威与 AI 创作两层。程序独占 World Engine、Event Director、检定、随机数、结果档位和写回；AI 在 `Narrative Envelope` 内创造具体现场、NPC 表现与自然行动提案，越权内容会被拒绝。v0.4.1 起，普通事件会通过 `SceneState` 保留现场事实、未解问题与上一轮结果，能够自然持续多轮，也能在玩家离开或问题解决时一轮结束。没有 AI 或连接失败时，系统仍会生成可继续的本地现场与具体结算。亚索作为首个持续运行的 Hero Actor，拥有位置、目标、行动、关系与重要记忆，但不能替玩家解决重大线程。
