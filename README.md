...
当前版本：**v0.9.2**。版本创建、离线备份与安全恢复方法见 [版本说明.md](版本说明.md)。当前功能、技术结构、网页端接口约定和完整更新日志可在 **API节点管理器 → 项目总览与日志** 中查看，也可由 `GET /api/admin/project-status` 读取。

阶段5已将叙事权限拆成程序权威与 AI 创作两层。程序独占 World Engine、Event Director、检定、随机数、结果档位和写回；AI 在 `Narrative Envelope` 内创造具体现场、NPC 表现与自然行动提案，越权内容会被拒绝。v0.5.0 起，角色出生后先经历帕拉斯短开场并获得可追踪信息；地图显示地点定位、风险、常见反馈与已知方向，追踪意图会进入 Director 和 AI Context，Scene 结果会把新发现与新的地点方向立即写回地图和旅途日志。没有 AI 或连接失败时，这条反馈循环仍能由本地叙事稳定完成。
...
- 每季 4 点行动点；耗尽后下一次行动自动推进到新季节并恢复
- 第一章采用一年四季制：春夏秋冬每季恰好 4 次行动，共 16 次行动；最后 4 次为固定终章
- 终章依次收束灵体异常、亚索同行与诺克萨斯残军三条线，最终战后显示试玩总结与第二章伏笔
...
- AI 在程序给定的叙事信封内动态生成现场与行动语义，程序随后完成能力、检定与难度映射
- 亚索 Hero Actor 会沿合理路线自主调查诺克萨斯活动，并以痕迹、传闻、远望、互动或合作形式进入事件
...
需要对比旧版时，双击 **`历史版本试玩器.cmd`**，输入列表序号或例如 `v0.7.0` 的版本号。试玩器会在项目外准备独立代码与存档副本，并使用 `http://127.0.0.1:8100`，不会切换当前 Git 分支，也不会覆盖当前开发版本。结束时双击 **`停止历史版本试玩.cmd`**。
