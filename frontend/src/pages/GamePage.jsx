...
export default function GamePage({ game, world, event, result, tab, busy, eventState, onTab, onTravel, onChoice, onDialogue, onCloseEvent }) {
  ...
  return <main className="game-page page-enter">
    ...
    <section className="game-content">
      ...
      {tab === 'map' && <WorldMap locations={world.locations} current={game.location} points={game.action_points} time={game.time} chapterComplete={game.chapter_complete} busy={busy} onTravel={onTravel}/>} 
...
function ResolutionPanel({ resolution, npcs }) {
  ...
  return <section className="resolution-panel">
    ...
    {resolution.outcome && <div className={`outcome-check ${resolution.outcome.code}`}><div><span>{resolution.outcome.check}</span><b>{resolution.outcome.label}</b></div><p>判定值 {resolution.outcome.roll} · 成功线 {resolution.outcome.chance}%</p><small>{resolution.outcome.code === 'failure' ? '能力或判断未能通过检定' : resolution.outcome.code === 'costly' ? '完成目标，但必须支付代价' : '检定通过，获得对应收益'}</small></div>}
...
function WorldMap({ locations, current, points, time, chapterComplete, busy, onTravel }) {
...
function Status({ player, game }) {
  return <div className="status-view"><div className="section-heading"><p>普通人的传说</p><h3>{player.name}的人物档案</h3></div>
    ...
    <h4>基础属性</h4><div className="stat-grid">
    ...
    <h4>五维人格</h4><div className="value-bars">{Object.entries(player.personality).map(([key,value]) => <ValueBar key={key} label={personalityNames[key]} value={value}/>)}</div>
    <h4>命运权重 <small>影响对应事件的出现概率</small></h4><div className="value-bars fate-bars">{Object.entries(player.fate_weights).map(([key,value]) => <ValueBar key={key} label={fateNames[key]} value={value}/>)}</div>
...
function EventSheet({ event, busy, eventState, onChoice, onClose }) {
  ...
  return <div className="sheet-backdrop"><section className={`event-sheet ${event.type === '战斗' ? 'battle' : ''}`}>
    ...
    {event.type === '战斗' && <div className="battle-warning"><Sword size={17}/><span>危险临近。你的基础属性、性情与装备将共同影响这一战。</span></div>}
    <div className={`event-choices ${eventState === 'CHOICES_AVAILABLE' ? 'available' : ''}`}>{event.choices.map((choice, index) => <button style={{animationDelay:`${index * .08}s`}} disabled={busy || eventState !== 'CHOICES_AVAILABLE'} onClick={() => onChoice(index)} key={choice.text}><span>{String.fromCharCode(65+index)}</span><div className="choice-copy"><b>{choice.text}</b><small>{choice.hint}</small><div className="choice-assessment"><em className={`risk-${choice.assessment.risk}`}>风险 · {choice.assessment.risk}</em><em>{choice.assessment.check}</em><em>{choice.assessment.forecast} · {choice.assessment.chance}%</em></div></div><ChevronRight size={18}/></button>)}</div>
