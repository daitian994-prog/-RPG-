...
function ResolutionPanel({ resolution, npcs }) {
  ...
  return <section className="resolution-panel">
    ...
    {resolution.worldFeedback?.worldChanged && <div className="world-feedback"><span>世界已记住这次选择</span>{resolution.worldFeedback.newPlayableSituation && <p>{resolution.worldFeedback.newPlayableSituation}</p>}</div>}
...
function WorldThreads({ game, busy, onInterveneThread, onFocusWorldTopic }) {
  ...
  return <>
    ...
    <h4>旅途日志 <small>记录你知道什么，不是强制任务</small></h4><div className="journey-log">{known.map(thread=><div key={thread.id}><b>{thread.title}</b><span>{thread.knownText || '尚未确认的传闻'}</span>{thread.worldEffects?.map(effect=><em key={effect}>世界余波 · {effect}</em>)}{focusedTopics.includes(thread.id) && <em>你正在留意这件事</em>}</div>)}</div>
    {debugMode && <><h4>开发信息 <small>完整状态链</small></h4><details className="world-thread-debug"><summary>展开最近一次内部状态</summary><div><pre>{JSON.stringify({worldState,directorState,lastResolution:game.last_resolution,ai:game.aiNarratorDebug,stateChangeLog:game.stateChangeLog?.slice(-8)}, null, 2)}</pre></div></details></>}
  </>
}
