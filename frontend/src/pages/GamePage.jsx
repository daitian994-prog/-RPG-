...
function ResolutionPanel({ resolution, npcs }) {
  ...
  return <section className="resolution-panel">
    ...
    {resolution.worldFeedback?.thread && <div className="world-feedback"><span>世界已记住这次选择</span><b>认知 {resolution.worldFeedback.thread.before.awareness} → {resolution.worldFeedback.thread.after.awareness} · 紧迫度 {resolution.worldFeedback.thread.before.urgency} → {resolution.worldFeedback.thread.after.urgency}</b>{resolution.worldFeedback.newPlayableSituation && <p>{resolution.worldFeedback.newPlayableSituation}</p>}</div>}
    {resolution.worldFeedback?.hero && <div className="world-feedback"><span>英雄关系已写回</span><b>{resolution.worldFeedback.hero.id} · {resolution.worldFeedback.hero.delta>=0?'+':''}{resolution.worldFeedback.hero.delta} · {resolution.worldFeedback.hero.stage}</b></div>}
  </section>
}
...
function WorldThreads({ game, busy, onInterveneThread, onFocusWorldTopic }) {
  const { worldState, directorState } = game
  ...
  return <>
    ...
    <h4>旅途日志 <small>记录你知道什么，不是强制任务</small></h4><div className="journey-log">{known.map(thread=><div key={thread.id}><b>{thread.title}</b><span>{thread.resolved ? thread.resolvedOutcome.label : thread.awarenessSignals.filter(item=>item.stage<=thread.stage).at(-1)?.text || '尚未确认的传闻'}</span>{thread.worldEffects?.filter(()=>thread.stage>=4||thread.resolved).map(effect=><em key={effect}>世界余波 · {effect}</em>)}{directorState?.focus?.includes(thread.id) && <em>正在关注 · 相关事件权重提高25%</em>}</div>)}{worldState.followUpHooks?.map(hook=><div key={hook}><b>未解决的问题</b><span>{hook}</span></div>)}{Object.entries(game.heroRelationships || {}).map(([id,relation])=><div key={id}><b>英雄相逢 · {id}</b><span>关系阶段 {relation.stage} · 记忆 {relation.history.length} 次</span></div>)}</div>
    <h4>完整闭环 Debug <small>World → Director → Check → Outcome → AI</small></h4><details className="world-thread-debug"><summary>展开最近一次完整状态链</summary><div><b>Tension · {directorState?.tension ?? 0} / 100</b><pre>{JSON.stringify({worldThread:worldState,director:directorState,selector:directorState?.lastSelection,check:game.last_resolution?.outcome,outcome:{worldFeedback:game.last_resolution?.worldFeedback,changes:game.last_resolution?.changes},ai:game.aiNarratorDebug,stateChangeLog:game.stateChangeLog?.slice(-8)}, null, 2)}</pre></div></details>
  </>
}
