...
export default function App() {
  ...
  const interveneThread = async (threadId, strategy) => {
    if (busy) return
    setBusy(true); setError('')
    try { const data = await api.interveneThread(game.id, threadId, strategy); setGame(data.game); setResult(data.intervention.message) }
    catch (e) { setError(e.message) } finally { setBusy(false) }
  }
